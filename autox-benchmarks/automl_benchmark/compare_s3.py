...
from automl_benchmark.s3_benchmark_upload import try_get_s3_object_bytes
from automl_benchmark.s3_client import make_s3_client, s3_cfg_usable

_BATCH_ID_RE = re.compile(r"^\d{8}T\d{6}Z$")

DEFAULT_CACHE_DIR = Path.home() / ".cache" / "autox-benchmarks" / "compare"
...
def storage_from_credentials(ini_cfg: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    if not bucket:
        raise ValueError("credentials.ini [storage] train_data_bucket_name is required")
