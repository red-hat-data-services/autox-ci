...
def upload_batch_aggregated(
    *,
    ...
    joined_results_key = _join_key(settings.benchmark_s3_prefix, "joined_results.csv")
    ...
    try:
        for s3_name, ir_path in _aggregated_pipeline_ir_uploads(settings):
            ...
            put_s3_bytes(
                s3_cfg=s3_cfg,
                bucket=bucket,
                key=_join_key(agg_prefix, s3_name),
                body=body,
            ...
            logger.info("Uploaded pipeline IR to s3://%s/%s", bucket, _join_key(agg_prefix, s3_name))
