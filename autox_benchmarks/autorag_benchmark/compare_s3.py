"""S3 helpers for AutoRAG benchmark compare UI: list batches, fetch CSVs, local cache."""
...
DEFAULT_CACHE_DIR = Path.home() / ".cache" / "autox_benchmarks" / "compare_rag"
...
def joined_results_key(benchmark_prefix: str) -> str:
    """S3 key for cumulative joined_results.csv (if we implement it)."""
    return f"{benchmark_prefix.strip('/')}/joined_results.csv"


def benchmark_runs_key(benchmark_prefix: str, batch_id: str) -> str:
    """S3 key for batch benchmark_runs.csv (AutoRAG uses this instead of merged_leaderboards)."""
    return f"{benchmark_prefix.strip('/')}/{batch_id}/aggregated/benchmark_runs.csv"
...
def list_batch_ids(
    *,
    ...
    """List batch_id folders that have aggregated/benchmark_runs.csv."""
    import botocore

    client = make_s3_client(s3_cfg)
    prefix = benchmark_prefix.strip("/") + "/"
    runs_suffix = "/aggregated/benchmark_runs.csv"
    found: set[str] = set()
    ...
    try:
        for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
            for obj in page.get("Contents") or []:
                ...
                if not key.endswith(runs_suffix):
                    continue
                rest = key[len(prefix) : -len(runs_suffix)]
                batch_id = rest.strip("/")
...
def fetch_joined_results(
    *,
    ...
    """Fetch cumulative joined_results.csv (if it exists)."""
...
def fetch_benchmark_runs(
    *,
    ...
    """Fetch batch benchmark_runs.csv (AutoRAG equivalent of merged_leaderboards)."""
    key = benchmark_runs_key(benchmark_prefix, batch_id)
    return fetch_csv_cached(
        s3_cfg=s3_cfg,
        bucket=bucket,
        key=key,
...
# Alias for compatibility with compare app naming
fetch_merged_leaderboards = fetch_benchmark_runs
