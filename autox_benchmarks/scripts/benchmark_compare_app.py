...
def main() -> None:
    ...
    st.title("Benchmark results compare")
    ...
    with tab_overview:
        ...
        if matched_combined.empty:
            st.info("No matching dataset_name + model rows between baseline and compare.")
        else:
            display_cols = [
                c
                for c in (
                ...
                if c in matched_combined.columns
            ]
            st.dataframe(matched_combined[display_cols], use_container_width=True, hide_index=True)
    ...
    with tab_heatmap:
        ...
        st.caption("Rows = datasets, columns = models. Compare heatmap uses the same row/column order as baseline.")
    ...
    with tab_coverage:
        ...
        if len(compare_dfs) == 1:
            ...
            with col_a:
                st.markdown("**Baseline only** (dataset + model)")
        ...
        if coverage_rows:
            ...
            fig_cov.update_layout(
                title="Dataset coverage (% baseline datasets with ≥1 matched model)",
                yaxis_title="Percent",
            )
