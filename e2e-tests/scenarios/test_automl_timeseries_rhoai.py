"""Run the time series AutoML training pipeline on OpenShift AI (KFP v2) with progress polling."""
...
from tests.lib.config_loaders import AutomlTimeseriesTestConfig, get_automl_timeseries_configs_for_run
from tests.lib.data_resolution import resolve_automl_dataset_s3_location
from tests.lib.integration_failures import require_rhoai_automl_env
from tests.lib.kfp_progress import run_succeeded, wait_for_run_with_progress
from tests.lib.settings import get_rhoai_automl_config, get_test_data_source_defaults

CONFIGS_FOR_RUN = get_automl_timeseries_configs_for_run()
...
class TestAutomlTimeseriesRhoaiKfp:
    """Submit time series AutoML runs using a precompiled ``pipeline.yaml`` (local path or downloaded)."""

    def test_pipeline_run_succeeds(
        self,
        test_config: AutomlTimeseriesTestConfig,
