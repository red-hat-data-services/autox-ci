"""Functional tests for AutoGluon timeseries training pipeline on RHOAI.

Test scenarios are defined in configs/timeseries_test_configs.json. Each scenario
uploads a dataset CSV, submits a pipeline run, and validates the result.
Filter by tags with AUTOML_FUNCTIONAL_TESTS_TAGS (e.g. smoke, timeseries).
...
from .configs.configs import AutoMLTimeseriesFunctionalConfig, get_timeseries_configs_for_run
...
TIMESERIES_CONFIGS = get_timeseries_configs_for_run()
...
class TestAutoMLTimeseriesFunctional:
    """Functional tests for AutoGluon timeseries training pipeline."""

    @pytest.mark.parametrize(
        "test_config", TIMESERIES_CONFIGS, ids=[c.id for c in TIMESERIES_CONFIGS]
    )
